...
std::vector<vector<Prediction>> Classifier::Classify(const cv::Mat& img, int N) {
    ...
    {
        int tempN = std::min<int>(labels_[i].size(), N);

        std::vector<int> maxN = Argmax(output[i], tempN);
        std::vector<Prediction> prediction;
        for (int j = 0; j < tempN; ++j) {
            int idx = maxN[j];
            prediction.push_back(std::make_pair(labels_[i][idx], output[i][idx]));
        }
...
int main(int argc, char** argv) {
    ...
    string model_file   = "dependence/deploy.prototxt";
    string trained_file = "dependence/model_iter_35000.caffemodel";
    string mean_file    = "dependence/standard_imagenet_mean.binaryproto";
    string label_file1   = "dependence/label1.txt";
    string label_file2   = "dependence/label2.txt";
    string label_file3   = "dependence/label3.txt";
   ...
   // label_file.push_back(label_file3);
    ...
    //string file = argv[6];
    string file ="/home/ao/QtPro/multiClassifier/Q7.jpg";
...
//  string label_file2   = argv[5];
  string model_file   = "dependence/deploy.prototxt";
  string trained_file = "dependence/model_iter_6000.caffemodel";
  string mean_file    = "dependence/standard_imagenet_mean.binaryproto";
  string label_file1   = "dependence/label1.txt";
  string label_file2   = "dependence/label2.txt";
  vector<string> label_file;
  ...
  //string file = argv[6];
  string file ="/home/ao/QtPro/multiClassifier/dependence/test.png";
